# LlamaIndex (Python) + Vantio Optics

LlamaIndex uses `httpx` — use the Python SDK `@shield` decorator (same as LangChain Python).

```bash
pip install vantio-agent-sdk llama-index
```

```python
from llama_index.llms.openai import OpenAI
from vantio import shield

@shield
async def ask(q: str):
    llm = OpenAI(model="gpt-4o-mini")
    return await llm.acomplete(q)
```

Fence: observe / optional Phantom Engine PII helpers. Not Phantom Engine host protection (kernel-level). Python `shield()` (3.0.2) records urllib and, when installed, httpx — which LlamaIndex uses.
