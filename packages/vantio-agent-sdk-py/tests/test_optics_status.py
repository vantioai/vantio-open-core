...
class HttpOutcomeTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_urlopen_connection_refused_stays_network_error(self) -> None:
        ...
        self.assertEqual(calls[0]["applicationOutcomeLabel"], "Connection to upstream service failed")
        self.assertEqual(calls[0]["applicationLabel"], "Connection to upstream service failed")
        self.assertEqual(calls[0]["failure_kind"], "connection")
