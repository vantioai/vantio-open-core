...
Optics intercepts via Node `fetch`, `undici.fetch`, `undici.request`, `undici.stream` / `pipeline` / `dispatch`, and `http`/`https` (`vantio run`) or Python `shield()` (urllib; requests/httpx/aiohttp when installed). That covers most real agents — and **can be skipped**:
